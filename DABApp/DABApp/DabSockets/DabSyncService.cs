...
//            var ql = await Service.DabService.InitializeConnection(token);
...
//                    ql = await Service.DabService.AddSubscription(1, "subscription { actionLogged { action { id userId episodeId listen position favorite entryDate updatedAt createdAt } } }");
//                    ql = await Service.DabService.AddSubscription(2, "subscription { tokenRemoved { token } }");
//                    ql = await Service.DabService.AddSubscription(3, "subscription { progressUpdated { progress { id badgeId percent year seen createdAt updatedAt } } }");
...
//                ql = await Service.DabService.AddSubscription(4, "subscription { episodePublished { episode { id episodeId type title description notes author date audioURL audioSize audioDuration audioType readURL readTranslationShort readTranslation channelId unitId year shareURL createdAt updatedAt } } }");
//                ql = await Service.DabService.AddSubscription(5, "subscription { badgeUpdated { badge { badgeId name description imageURL type method data visible createdAt updatedAt } } }");
