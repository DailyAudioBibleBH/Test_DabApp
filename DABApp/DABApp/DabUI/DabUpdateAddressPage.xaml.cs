...
namespace DABApp
{
    public partial class DabUpdateAddressPage : DabBaseContentPage
    {
        ...
        public DabUpdateAddressPage(DabGraphQlAddress address, Dictionary<string, string> countries, bool IsShipping)
        {
            ...
            {
                Country.SelectedItem = countryDictionary.FirstOrDefault(x => x.Key == address.country).Value;

                object newCountry = address.country;
                string countryCode = countryDictionary.Where(x => x.Key == newCountry.ToString()).ToList().FirstOrDefault().Key;
        ...
        async void OnSave(object o, EventArgs e)
        {
            if (Validation())
            {
                ...
                update.email = dbSettings.GetSetting("Email", "");
                ...
                update.country = countryDictionary.FirstOrDefault(x => x.Value == Country.SelectedItem.ToString()).Key;
        ...
        void OnCountrySelected(object o, EventArgs e)
        {
            ...
            if (RegionLabel.Text == null) RegionLabel.Text = "Region";
        }
        ...
        bool Validation()
        {
            ...
            if (Country.SelectedItem != null)
            {
                ...
                var country = countryDictionary.FirstOrDefault(x => x.Value == selected.ToString());
                ...
                //countries with regions rule
                if (stateDictionary.Keys.Contains(country.Key))
                {
                    var regions = stateDictionary[country.Key];
                    ...
                    if (regions != null)
                    {
                        ...
                        {
                            result = false;
                            RegionWarning.Text = $"{RegionLabel.Text} is required for {country.Value}.";
