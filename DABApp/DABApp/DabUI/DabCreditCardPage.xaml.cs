...
namespace DABApp
{
    public partial class DabCreditCardPage : DabBaseContentPage
    {
        ...
        void CardNumber_TextChanged(System.Object sender, Xamarin.Forms.TextChangedEventArgs e)
        {
            if (CardNumber.IsEnabled) //only apply when adding a card
            {
                if (CardNumber.Text.StartsWith("37") || CardNumber.Text.StartsWith("34"))
                {
                    //american express
